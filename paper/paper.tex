...
error, an upper bound on the relative error between a real number and its floating-point representation.
% \camille{Given the target journal where any readers (like myself :) ) won't be expert in nuemrical perturbations, I think that you could add a bit more details here on why those numbers are good to simulate machine errors}.
...
identical results with the same checksums. For SPM, the files (as measured by checksums) were different but differences
...
subject-level unthresholded t-statistic maps computed by the mean across all subjects.
