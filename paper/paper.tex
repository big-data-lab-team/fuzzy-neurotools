...
\maketitle
\begin{abstract}
  Variability has been broadly observed in functional MRI analyses as a result of differences
  both between and within analytic tools. However, the
  relationship between within-tool and between-tool variabilities for a
  ...
  observed differences to within-tool variability
  simulated using Monte-Carlo arithmetic. In group analyses, we found that
  between-tool variability was consistently larger than within-tool
  variability simulated from numerical perturbations of the magnitude of
  machine error. In subject analyses, the same within-tool variability
  approached between-tool variability in some regions for some subjects.
  Interestingly, within- and between-tool variabilities appeared moderately
  correlated ($r > 0.5$) in all tested conditions. Finally, we found that
  within-tool variability had effects of similar magnitude to between-tool
  variability when simulated from numerical perturbations introduced at the virtual
  precision of 17 bits --- a high but plausible level of uncertainty for
  scientific computing libraries \gk{What does this mean? They only have
  precision of 17, or they use data that is 17 bits?}.
   ...
   error} \tristan{Better?} Our findings motivate the
  continued investigation of within-tool variability in neuroimaging, and position it
...
\tristan{Ali, can you update the figures to (1) show WT variability for each tool instead of for tool pairs (2) remove difference between BT and WT (3) show the XY plot as a joint histogram?}
...
moderately correlated at this precision \tristan{add r score}.
...
than WT.
%\gk{what regions? This kind of came from nowhere}\tristan{I meant "at the edges", fixed.}
