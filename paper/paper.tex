...
\begin{abstract}
 ...
 We extended
  ...
  correlated ($r > 0.5$ across voxels) in all tested conditions. 
...
average difference across the three Fuzzy Libmath samples.
...
Mean of differences in t-statistic maps is generally centered around zero for BT and WT
(one-sample t-test p~\textless~$10^{-4}$ for all tests).
However, for each tool pair (A, B), BT variability was
significantly larger than machine error in tool A or B,
with an order of magnitude larger standard deviation of differences 
in both thresholded and unthresholded maps, 
(Wilcoxon signed-rank test and t-test p~\textless~$10^{-4}$ for all tests).
...
in [0.68, 0.69], p$< 10^{-5}$, Figure~\ref{fig:unthresh-maps}-\textbf{C}). 
...
\begin{table}[h]
  ...
  \begin{tabular}{cccccc|cc}
    ...
    {Within Tool}     & FSL          & -0.003                           & 0.069                              & -0.010                             & 0.103    & -0.002 & 0.099    \\
    {(WT)}            & SPM          & 0.001                           & 0.097                              & 0.000                             & 0.076    & 0.000 & 0.064    \\
    {}                & AFNI         & 0.002                           & 0.228                              & -0.001                             & 0.178    & 0.000 & 0.178    \\
    \bottomrule
  \end{tabular}
...
signed-rank test and t-test p~\textless~$10^{-4}$ for 12 subjects).
...
BT in some regions (Figure~\ref{fig:unthresh-maps-sbj}). As for the group maps, BT and WT
 appeared moderately correlated for all subjects (r in [0.52,
    0.69], p$< 10^{-5}$ for all subjects, see Supplemental Material~\ref{sec:supp-subjects} and~\ref{sec:supp-worst-subject}).

%%%%%%%%%% Var. of Unthresh sbj05%%%%%%%%
\begin{figure*}[ht]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/plots/Bland-Altman/unthresh-sbj10.png}
...
\begin{figure*}[ht]
  \centering
  \includegraphics[width=\textwidth]{figures/10-marked.pdf}
