...
\author{Ali Salari$^1$, Yohan Chatelain$^1$, Camille Maumet$^3$, Gregory Kiar$^4$, Tristan Glatard$^1$\\
  \vspace*{0.1cm}\\
...
A floating-point number is represented by a finite number of bits, 24 bits
for single-precision and 53 bits for double-precision in the IEEE-754 standard \tristan{This is the size of the mantissas, not the size of the entire representation}.
Machine error, the smallest difference between two floating-point numbers that can be represented,
is $2^{-24}$ in single-precision and $2^{-53}$ in double precision \tristan{The magnitude of machine error depends on the exponent}.
We simulated machine error by instrumenting the last precision bits \tristan{We cannot ``instrument'' a bit, we can perturb it. Code is instrumented.}
...
%an upper bound on the relative error between a real number and its floating-point representation.
