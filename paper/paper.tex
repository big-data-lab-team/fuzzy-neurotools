...
\begin{abstract}
  ...
  given analysis is unclear \camille{early on in the abstract, I think that it would be worth specifying what within-tool covers in the context of this paper: i.e. a focus on variations that can be induced by low level libraries as seen for example when using different operating systems -- which is different from within-tool variability that could be induced when using different parameters (e.g smoothing 6mm or 8mm) or algorithms (e.g. FLIRT versus FNIRT in FSL) within the same software } . We extended a previous comparison of fMRI
  ...
  correlated ($r > 0.5$) \camille{maybe add ``across voxels or across xxx'' so that we understand what the correlation is measuring a bit better?} in all tested conditions . 
  ...
  as a possible proxy for studying some of the arbitrary between-tool software variations. \camille{should we add ``for subject-level analyses'' here? Since we said earlier that within-tool was consistently smaller than between tool variability for group studies.}
\end{abstract}
...
role in the reproducibility of \camille{add ``an'' ?} fMRI analysis overall. 
...
\begin{table}[h]
  ...
  \begin{tabular}{|c|l|c|c|c|}
    ...
    \hline
  \end{tabular}
  \caption{Software processing steps (adapted from~\cite{bowring2019exploring}). \camille{``Analysis Voxel Size'' is not a processing step per se but more a parametrization of inter-subject registration so maybe it should be removed? // Not sure if this is worth noting but in FSL ``Inter-subject registration'' happens after the first-level analysis}}
...
in~\cite{bowring2019exploring} except for SPM for which we \camille{add ``had to use a more recent version in order to be able to use it with''} GNU/Octave
...
error \camille{Given the target journal where any readers (like myself :) ) won't be expert in nuemrical perturbations, I think that you could add a bit more details here on why those numbers are good to simulate machine errors}. The resulting samples are equally plausible estimates of
...
% Moreover, to evaluate numerical perturbations at different magnitudes,
...
\section{Results}
The scripts, Docker images, and data \camille{maybe a detail but consider using something like ``and derived datasets that were used to reproduce the results'' ? As it is now, it made me wonder whether the whole nifti dataset was in the github repo} to reproduce the results are available
...
identical results \camille{It would be nice here to be more specific about how you checked that they were identical -- using a sha sum ? And maybe even insisting by using ``striclty identifical''. I think for different people in the community ``identical'' can mean different things}. For SPM, the files (as measured by checksums) were different but differences
were visually unnoticeable. For AFNI, the activation maps were similar
overall, however, differences were noticeable visually.
The observed differences remained small \camille{maybe also add ``and the overall patterns of activations were preseved''} (see Supplemental Material~\ref{sec:supp-repro}), and might be due to the use of
...
group-level t-statistics. For each tool pair (A, B), BT \camille{could we restate which metric we are looking at here -- as it stands BT means between-tool but coul represent many different metrics? It is ``absolute difference BT''?} was
...
\begin{table}[h]
  ...
  \caption{Voxel-wise mean and standard deviation of BT and WT variability
    in t-statistics maps \camille{here and in other places -- is it t-statistic maps? (i.e. without s after statistic)}.}
...
subject-level unthresholded t-statistics maps \camille{Is this for one subject in particular ? If so how was it chosen ?Or a mean over all subjects?}. As for group-level maps,
...
WT and BT for each subject \camille{across voxels?}, showing a consistent moderate correlation between BT and
WT.
