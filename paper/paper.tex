...
\begin{abstract}
  ...
  variabilities \camille{I did not quite understand what `masking instability` meant here, maybe ``differences observed in brain masks?''}. Our findings motivate the continued investigation of
...
\begin{table}[h]
  ...
  % \camille{``Analysis Voxel Size'' is not a processing step per se but more a parametrization of inter-subject registration so maybe it should be removed? // Not sure if this is worth noting but in FSL ``Inter-subject registration'' happens after the first-level analysis}}
...
% Moreover, we computed WT variability for a pair of tools as the sum of WT absolute differences for each tool.
% All differences were computed over the intersection of t-statistic maps between the pair of tools.
...
in both thresholded and unthresholded maps (t-test p~\textless~$10^{-5}$ \camille{I don't understand the `centered around zero' and then the very small p-value, is this contradictory? } \camille{I am no statistician but I think we cannot perform t-test to test for differences in t-statistic. To perform a parametric test we would have to transform to z-stat or to go back to the contrast values. This reminds me of how we do tests in meta-analysis, more on page 3 of (unpublished) \url{https://github.com/cmaumet/zmeta_paper/blob/master/ibma_zmeta_journal.pdf}. I think the right thing to do here would be to use some version of Stouffer, if that's of interest I am happy  to chat about it? Hopefully this should not be too cumbersome, mainly T to Z transfo followed by some averaging and then testing against a normal distribution. Let me know!} ).
...
\begin{figure*}[ht]
  ...
  \caption{Voxel-wise comparison of group-level differences in BT and WT. \camille{That's not obvious to me why are the bins bigger here compared to the B-A plots? }}
...
\camille{to me, this seems like an important result, is this big difference also visible on the thresholded maps of that subject? if so, that's just a thought of course, but I think that it could be very nice to also display the thresholded brain maps that correspond to the subject in question (similar to figure 4). This would be an important piece of info for brain imagers to understand the impact WT variability can have on their subject-level results.}
...
than WT (Figure~\ref{fig:thresh-maps}-\textbf{A},\textbf{B},\textbf{C} and Table~\ref{table:pipeline-stats}) \camille{I personnally really like Fig. 4! It shows very nicely how instable some clusters are and this is close to what brain imagers are used to. Maybe it could be in the main text instead of supplementary?}. 

Moreover, to compare the effects of BT and WT on the detection of activated
regions, we measured WT instability \camille{Could we describe in a sentence what WT instability mean in this context? i.e. when is it zero and when is it one. Sorry if I missed it!} and BT instability in each region of
...
were removed after fixing masks \camille{maybe change `fixing masks` into `when using the intersection mask for all maps` or something similar to avoid making it sound like what was done previously could have been fixed?.} (Figure~\ref{fig:unthresh-maps-mask}-\textbf{A},\textbf{B}).
