...
A floating-point number can be represented by a finite number of precision bits, 24 bits
for single-precision and 53 bits for double-precision in the IEEE-754 standard.
Thus, the machine error, the smallest difference between two floating-point numbers that the computer
can recognize, for single-precision is $2^{-24}$ and double-precision is $2^{-53}$.
We simulated the machine error by instrumenting the last precision bits of the floating-point numbers.
...
strictly identical results with the same MD5 checksums. For SPM, the files (as measured by checksums) were different but differences
