...
\begin{table}[h]
  ...
  \begin{tabular}{|c|l|c|c|c|}
    ...
    \hline
  \end{tabular}
  \caption{Software processing steps (adapted from~\cite{bowring2019exploring}). \camille{I don't fully understand the "first-level" and "second-level" info in this table. Do to the "contrasts" the model has to be estimated so each time "Contrast" is ticked then "model estimation" should be ticked as well}}
...
across a given pair of tools \camille{standard dev with 2 samples ? --> cf. discussion on mattermost).
